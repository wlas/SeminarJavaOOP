    ...
    public void showTaskScheduler(TaskSchedulerList taskSchedulerList) {
        if (taskSchedulerList != null) {
            ...
            for (TaskScheduler task : taskSchedulerList.getTaskSchedulersList()) {
                sb.append(
                        String.format(
                                "ID: %s | Content: %s | Date Add: %s | Deadline: %s | Date Of Completion: %s | Priority: %s | Status task: %s.\n",
                                task.getTaskID(), task.getTaskContent(), task.getDateAdd(), task.getDeadlain(),
                                task.getDateOfCompletion(), task.getPriority(), task.getStatus()));
            }
